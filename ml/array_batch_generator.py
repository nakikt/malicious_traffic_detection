...
class ArrayBatchGenerator(keras.utils.Sequence):
    ...
    def reshape_batch_data(self, data, values):
        img_data = [np.reshape(f_data, (self.array_size, 1)) for f_data in data]
    ...
    def read_input(self, idx):
        ...
        with open(self.images_fname, 'rb') as f, open(self.labels_fname, 'rb') as g:
            f.seek(start_idx * self.array_size)
            g.seek(start_idx)
            while current_idx * self.array_size< end_idx * self.array_size:
                img_data_x = f.read(self.array_size)
                img_data_y = g.read(1)
                if len(img_data_x) == self.array_size and len(img_data_y) == 1:
